...
function BacklogItem(props) {
    ...
    return (
        <li className={` ${props.status === 'backlog' ? (descriptionIsOpen ? "w-full flex order-first justify-center" : "w-1/3 flex flex-row flex-wrap") : "flex flex-row flex-wrap w-full"} `}>
            
            <div className={` ${props.status === 'backlog' ? (descriptionIsOpen ? "w-2/5 justify-center mx-20" : "w-full") : "w-full justify-center" } border-2 border-gray-200 hover:border-gray-300 hover:font-semibold rounded m-1  px-10 items-center bg-white`}>
                ...
                {descriptionIsOpen ? (
                    <div className="border-t-2 text-center py-4">{props.description ? props.description: "no additional description"}</div>
                ) : null}
