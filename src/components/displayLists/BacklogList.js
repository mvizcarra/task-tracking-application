    ...
    return (
        <ul className="flex flex-wrap flex-row px-2 mb-0 mt-1">
            {props.tasks.map( (task) => (
                <BacklogItem key={task.id} id={task.id} name={task.name} description={task.description} status={task.status} openDeleteModal={openDeleteModalHandler} forwardId={forwardIdHandler}/>
            ))}
        </ul>
    );
}
